...
object Css {
  ...
  def selector(selector: String, children: CssChild*) =
    CssSelector(selector, children)
  def media(query: String, children: CssChild*) = CssMediaQuery(query, children)
  def pseudo(pseudoClass: String, children: CssChild*) =
    CssPseudoClass(pseudoClass, children)
  def cssClass(className: String, children: CssChild*) =
    CssSelector("." + className, children)
  def directChild(tagName: String, children: CssChild*) =
    CssSelector(">" + tagName, children)
  def mediaSmall(children: CssChild*) =
    CssMediaQuery("(max-width: 991px)", children)
  def mediaLarge(children: CssChild*) =
    CssMediaQuery("(min-width: 992px)", children)
  ...
  def evenChild(children: CssChild*) =
    CssPseudoClass("nth-child(even)", children)
  def oddChild(children: CssChild*) = CssPseudoClass("nth-child(odd)", children)
  def evenOfType(children: CssChild*) =
    CssPseudoClass("nth-of-type(even)", children)
  def oddOfType(children: CssChild*) =
    CssPseudoClass("nth-of-type(odd)", children)
  ...
  def firstOfType(children: CssChild*) =
    CssPseudoClass("first-of-type", children)
  def focus(children: CssChild*) = CssPseudoClass("focus", children)
  def hover(children: CssChild*) = CssPseudoClass("hover", children)
  def indeterminate(children: CssChild*) =
    CssPseudoClass("indeterminate", children)
  ...
  def placeholderShown(children: CssChild*) =
    CssPseudoClass("placeholder-shown", children)
  ...
  def firstLetter(children: CssChild*) =
    CssPseudoClass(":first-letter", children)
  ...
  def not(cssClass: CssClass, children: CssChild*) =
    CssPseudoClass("not(." + cssClass.name + ")", children)
  def not(pseudoClass: CssPseudoClass, children: CssChild*) =
    CssPseudoClass("not(:" + pseudoClass.name + ")", children)
}
...
object CssChild {
  ...
  def emitCssChildren(builder: StringBuilder,
    ...
    for (c <- flattened) c match {
      case CssPseudoClass(name, cs) =>
        emitCssChildren(builder, keyframes, media, selector + ":" + name, cs)
      case CssMediaQuery(query, cs) =>
        emitCssChildren(builder, keyframes, media + " " + query, selector, cs)
      case CssSelector(s, cs) =>
        emitCssChildren(builder, keyframes, media, selector + s, cs)
