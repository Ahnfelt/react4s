...
trait Component[M] {
  ...
  private[react4s] val attachedAttachables =
    scala.collection.mutable.ListBuffer[Attachable]()
  ...
  def attach[T <: Attachable](attachable: T): T = {
    attachedAttachables += attachable; attachable
  }
  ...
  object State {
    ...
    def of[T](initial: Signal[T]): State[T] =
      attach(new AttachableState(initial))
  }
}
...
object Component {
  ...
  def apply[M](f: { def apply(): Component[M] }) =
    ConstructorData(Constructor0(f))
  ...
  def apply[P1, M](f: { def apply(p1: P[P1]): Component[M] }, p1: P1) =
    ConstructorData(Constructor1(f, p1))
  ...
  def apply[P1, P2, M](f: { def apply(p1: P[P1], p2: P[P2]): Component[M] }, p1: P1, p2: P2) =
    ConstructorData(Constructor2(f, p1, p2))
  ...
  def apply[P1, P2, P3, M](f: {
    def apply(p1: P[P1], p2: P[P2], p3: P[P3]): Component[M]
  }, p1: P1, p2: P2, p3: P3) = ConstructorData(Constructor3(f, p1, p2, p3))
  ...
  def apply[P1, P2, P3, P4, M](f: {
    def apply(p1: P[P1], p2: P[P2], p3: P[P3], p4: P[P4]): Component[M]
  }, p1: P1, p2: P2, p3: P3, p4: P4) =
    ConstructorData(Constructor4(f, p1, p2, p3, p4))
...
object Signal {
  ...
  def of[T](getSample: Get => T): Signal[T] = new Signal[T] {
    def sample(get: Get) = getSample(get)
  }

  /** Create a signal that always returns the supplied constant when sampled. */
  def apply[T](constant: T): Signal[T] = new Signal[T] {
    def sample(get: Get) = constant
  }
}
...
abstract class JsComponent(componentClass: Any) {
  def apply(children: JsTag*) =
    JsComponentConstructor(componentClass, children, None, None)
}
...
abstract class Context[T](val defaultValue: T) {
  ...
  def provide(value: T, children: Node*): Node =
    ContextProvider(this, value, children: _*)
  def consume(body: T => Node): Node =
    ContextConsumer(this, value => body(value.asInstanceOf[T]))
}

/** Same as Context, but takes in a raw context created with React.createContext(...). */
abstract class JsContext[T](contextType: Any) {
  def provide(value: T, children: Node*): Node =
    ContextProvider(contextType, value, children: _*)
  def consume(body: T => Node): Node =
...
case class Style(name: String, value: String) extends Tag with CssChild {
  ...
  def url(value: String) =
    apply("url('" + value.replace("'", "\\'").replace("\n", "\\n") + "')")
  ...
  def rgb(r: Double, g: Double, b: Double) =
    apply("rgb(" + r.round + ", " + g.round + ", " + b.round + ")")
  def rgba(r: Double, g: Double, b: Double, a: Double) =
    apply("rgba(" + r.round + ", " + g.round + ", " + b.round + ", " + a + ")")
  ...
  def initial() = apply("initial")
  def unset() = apply("unset")
  def revert() = apply("revert")
  def all() = apply("all")
  def default() = apply("default")
  def visible() = apply("visible")
  def hidden() = apply("hidden")
  def capitalize() = apply("capitalize")
  def uppercase() = apply("uppercase")
  def lowercase() = apply("lowercase")
  def nowrap() = apply("nowrap")
  def pre() = apply("pre")
  def preLine() = apply("pre-line")
  def preWrap() = apply("pre-wrap")
  def underline() = apply("underline")
  def overline() = apply("overline")
  def lineThrough() = apply("line-through")
  def bold() = apply("bold")
  def italic() = apply("italic")
  def pointer() = apply("pointer")
  def collapse() = apply("collapse")
  def solid() = apply("solid")
  def dotted() = apply("dotted")
  def dashed() = apply("dashed")
  def inset() = apply("inset")
  def left() = apply("left")
...
object Style {
  ...
  def toStandardName(name: String): String =
    camelPattern.replaceAllIn(name, m => "-" + m.group(1).toLowerCase)
  def toReactName(name: String): String =
    snakePattern.replaceAllIn(name, m => m.group(1).toUpperCase)
}
...
private[react4s] case class Constructor1[P1, M](f: {
  def apply(p1: P[P1]): Component[M]
    ...
    extends Constructor[M](Seq(p1))
private[react4s] case class Constructor2[P1, P2, M](f: {
  def apply(p1: P[P1], p2: P[P2]): Component[M]
}, p1: P1, p2: P2)
    extends Constructor[M](Seq(p1, p2))
private[react4s] case class Constructor3[P1, P2, P3, M](f: {
  def apply(p1: P[P1], p2: P[P2], p3: P[P3]): Component[M]
}, p1: P1, p2: P2, p3: P3)
    extends Constructor[M](Seq(p1, p2, p3))
private[react4s] case class Constructor4[P1, P2, P3, P4, M](f: {
  def apply(p1: P[P1], p2: P[P2], p3: P[P3], p4: P[P4]): Component[M]
}, p1: P1, p2: P2, p3: P3, p4: P4)
    extends Constructor[M](Seq(p1, p2, p3, p4))
private[react4s] case class Constructor5[P1, P2, P3, P4, P5, M](f: {
...
object A extends CommonEvents {
  ...
  def onLeftClick(handler: MouseEvent => Unit) =
    EventHandler("onClick", { e: MouseEvent =>
  ...
  def allowFullScreen(value: String = "true") =
    Attribute("allowFullScreen", value)
  def allowTransparency(value: String = "true") =
    Attribute("allowTransparency", value)
  ...
  def contentEditable(value: String = "true") =
  ...
  def formNoValidate(value: String = "true") =
