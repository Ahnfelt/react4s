...
class ReactBridge(react: => Any,
  ...
  private def doAddStyle(name: String, css: String): Unit =
  ...
  private val addCss: String => Boolean = { cssName =>
    val result = !cssMap.contains(cssName); cssMap.update(cssName, true); result
  }
  ...
  def nodeToReact(node: Node): ReactElement = {
    node match {
      ...
      case ContextProvider(contextType, value, nodes @ _*) =>
        ...
        val binding =
          js.Dictionary[js.Any]("value" -> value.asInstanceOf[js.Any])
        React.createElement(context.Provider, binding, children: _*)
      case ContextConsumer(contextType, body) =>
        val context = getReactContext(contextType)
        React.createElement(context.Consumer, null, { value: js.Any =>
          nodeToReact(body(value))
        })
      case Portal(child, container) =>
        ReactDOM
          .createPortal(nodeToReact(child), container.asInstanceOf[js.Any])
  ...
  def createComponentClass(constructorData: ConstructorData[_]) = {
    ...
    dynamicConstructor.prototype.componentWillUnmount = { (self: js.Dynamic) =>
      ...
      for (attachable <- instance.attachedAttachables)
        attachable.componentWillUnmount(Get.Unsafe)
    }: js.ThisFunction
    ...
    dynamicConstructor.prototype.render = { (self: js.Dynamic) =>
      ...
      if (!isRenderingToString) {
        ...
        for (attachable <- instance.attachedAttachables)
          attachable.componentWillRender(Get.Unsafe)
      }
      instance.updateScheduled = false
      nodeToReact(instance.render(Get.Unsafe))
    }: js.ThisFunction
